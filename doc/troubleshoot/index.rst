...
Source file (~/openrc) 
----------------------
If you try run swift cli in controller nodes, please check OS_AUTH_URL is correct. If the value is ``http://<IP>:5000/``, please correct it to ``http://<IP>:5000/v2.0`` as following.
...
If there is a external load balancer in front of you Swift cluster, and you congiure the swift cluster with it. Please fill the external LB IP for ``Swift API IP Address`` in :ref:`plugin page <swift_api_ip_address>`.
...
If any client runs into ``401 Unauthorized`` issue, please use Swift CLI verity it again and make sure the settings of middlewares in Swift cluster are correct. 
