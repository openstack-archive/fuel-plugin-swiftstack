...
Also prepare two Fuel slave nodes:
    ...
    * 1 Compute Node (have **Compute** and **Storage - Cinder** roles)
    ...
    .. note::
        ...
        network (SwiftStack) outside of Fuel network, that prevents network starvation on Fuel 
        networks when Swift service deamons are moving data or clients send large data into Swift 
        ...
        Also, the SwiftStack Nodes need to communicate with the SwiftStack conroller, so please make
        user they can talked to each other, you can combined ``management (SwiftStack)`` to Fuel 
